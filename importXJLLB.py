...
for list in os.listdir(rootdir):
    #读取利润表文件
    i = 0
    df = pd.read_csv(rootdir+'/'+list+'/xjllb.csv',encoding = "GBK")
    ...
    for row in dt.iterrows():
        try:
            cursor.execute("insert into t_xjllb(code,date,xssptglwsddxj,khckhtycfkxjzje,xzyyhjkjzje,xqtjrjgcrzjjzje,sdybxhtbfqddxj,sdzbxywxjje,bhcjjtzkjzje,czjyxjrzcjzje,sqlxsxfjyjdxj,crzjjzje,hgywzjjzje,sddsffh,sddqtyjyhdygdxj,jyhdxjlrxj,gmspjslwzfdxj,khdkjdkjzje,cfzyyhhtykxjzje,zfybxhtpfkxdxj,zflxsxfjyjdxj,zfbdhldxj,zfgzgyjwzgzfdxj,zfdgxsf,zfdqtyjyhdygdxj,jyhdxjlcxj,jyhdcsdxjllje,shtzssddxj,qdtzsyssddxj,czgdzcwxzchqtcqzcsshdxjje,czzgsjqtyydwsddxjje,sddqtytzhdygdxj,jszyhdqckssddxj,tzhdxjlrxj,gjgdzcwxzchqtcqzcszfdxj,tzszfdxj,zydkjzje,qdzgsjqtyydwzfdxjje,zfdqtytzhdygdxj,zjzyhdqckszfdxj,tzhdxjlcxj,tzhdcsdxjllje,xstzsddxj,zgsxsssgdtzsddxj,qdjksddxj,fxzqsddxj,sdqtyczhdygdxj,czhdxjlrxj,chzwzfdxj,fpgllrhcflxszfdxj,zgszfgssgddgllr,zfqtyczhdygdxj,czhdxjlcxj,czhdcsdxjllje,hlbddxjjxjdjwdyx,xjjxjdjwjzje,qcxjjxjdjwye,qmxjjxjdjwye,jlr,ssgdsy,wqrdtzss,zcjzzb,gdzczjyqzczhscxwzzj,wxzctx,cqdtfytx,dtfydjs,ytfydzj,czgdzcwxzchqtcqzcdss,gdzcbfss,gyjzbdss,dysyzj,yjfz,cwfy,tzss,dysdszcjs,dysdsfzzj,chdjs,jyxysxmdjs,jyxyfxmdzj,ywgswjskdjs,yjsswwgkdzj,qt,jyhdcsxjllje,zwzwzb,ynndqdkzhgszq,rzzrgdzc,xjdqmye,xjdqcye,xjdjwdqmye,xjdjwdqcye,xjjxjdjwdjzje) "\
                           ...
                           % (list,row[0],\
                              ...
                              float(row[1][85] if row[1][85]!='--' else 0),float(row[1][86] if row[1][86]!='--' else 0),float(row[1][87] if row[1][87]!='--' else 0),float(row[1][88] if row[1][88]!='--' else 0)))
    ...
    print(list+'现金流量表表完成,共插入记录'+str(i)+'条')
    i=0
